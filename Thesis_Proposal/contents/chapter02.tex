...
其次是雙評價函數的「粗精結合」策略。研究分析發現不同的評價函數具備不同的曲線特性，為了應對局部極大值，系統將兩種函數結合使用：在粗對焦階段採用「方差函數（Variance）」，因其曲線峰值範圍較寬且在遠離焦點處較為平滑，有利於在大範圍內尋找主峰方向；在精對焦階段則切換至「Brenner 梯度函數」，其曲線極其陡峭且靈敏度高，能精確鎖定真正的全域最大值。如圖 \ref{fig:mcs_metrics} 所示，這套邏輯確保了系統能在不同焦距範圍下切換最適用的評價指標。
...
在傳統光學系統中，樣本的高頻資訊會因繞射角過大而無法進入物鏡孔徑。該技術的核心於：當照明光穿過擴散片後，會產生隨機的相位調控（Random Phase Modulation），這相當於在空間頻域上對樣本進行了寬頻編碼。

擴散片產生的隨機散斑（Speckle）包含豐富的空間頻率成分。透過平移擴散片以產生一系列不同的隨機圖案照明，每一幀影像都包含了樣本不同頻譜區域的疊加資訊。這種方法不僅大幅簡化了硬體架構（無需精密的 LED 陣列控制），更利用了隨機圖案在頻域上的全覆蓋特性。演算法便能從中透過疊代運算，將這些被編碼在高頻中的細節還原出來，如圖 \ref{fig:pifp_setup_bf} 所示。
...
傳統的 FP 依賴 LED 陣列進行角度掃描，且必須建立在光線是「相干的（Coherent）」（如雷射或窄頻 LED）前提下。然而，螢光發光是一種「非相干（Incoherent）」且自發各向同性的過程，其相位資訊在空間與時間上均不具備相干性，因此傳統基於相位復原的 FP 無法直接應用於螢光成像。

為此，Dong 等人提出了 \textbf{圖案化照明傅立葉疊層 (piFP)} \cite{dong2014high}。piFP 的核心邏輯在於將非相干成像系統建模為強度上的線性疊加。系統不再進行角度掃描，改為在樣本前方放置一個能夠產生空間高頻圖案（例如散斑 Speckle）的元件，其具體實作之系統架構如圖 \ref{fig:pifp_setup_fluo} 所示。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/ch2/fig2_12_pifp_setup_fluo.png}
    \caption{針對螢光系統的 piFP 實作架構示意圖，展示了透過擴散片產生隨機結構化照明的配置。}
    \label{fig:pifp_setup_fluo}
\end{figure}

在該架構中，利用高頻照明圖案與樣本結構的頻率混疊（Aliasing）原理，可以將樣本原本位於截止頻率外的高頻資訊有效「搬移（Shift）」至物鏡可接收的低頻範圍內。這種物理編碼過程的數學原理可由頻譜搬移理論進行解釋，如圖 \ref{fig:pifp_concept} 所示。透過不同空間頻率成分的混疊，低 NA 的物鏡得捕捉到被編碼後的高解析度影像資訊，進而在後端透過演算法還原出樣本的精細結構。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/ch2/fig2_13_pifp_concept.png}
    \caption{piFP 突破繞射極限之原理示意圖（圖片來源：\cite{dong2014high}）。透過照明圖案與樣本結構的頻率混疊，實現頻譜位移並擴展系統的有效截止頻率。}
